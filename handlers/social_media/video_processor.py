...
from config import TEMP_DIRECTORY, PLATFORM_IDENTIFIERS, DATABASE_CHANNEL_ID
...
async def process_social_media_video(message, bot, url, platform_name, progress_msg=None):
    ...
    try:
        ...
        # Backup to database channel (if configured)
        if DATABASE_CHANNEL_ID:
            try:
                backup_file_name = f"{platform_name.lower()}_video_backup.mp4"
                backup_doc = FSInputFile(temp_video_path, filename=backup_file_name)
                caption = (
                    f"{platform_name} backup\n"
                    f"User ID: {message.from_user.id}\n"
                    f"Username: @{message.from_user.username}" if message.from_user.username else f"User ID: {message.from_user.id}"
                )
                ...
                logger.info(f"Backup sent to database channel {DATABASE_CHANNEL_ID}")
            except Exception as backup_error:
                logger.warning(f"Failed to send backup to database channel: {backup_error}")
